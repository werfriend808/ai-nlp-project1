...
_JUNK_SECTION_RE = re.compile(
    ...
    r"|#\S+(?:\s+#\S+){1,}"
    # 2026-08-31 추가: 해시태그가 2개 이상 연달아 나오는 패턴만 잡다 보니 "#금리 금리
    # 더보기"처럼 해시태그 1개짜리 "관련 키워드 더보기" 위젯을 놓치는 사례가 실측
    # 확인됨(row633: 가계빚 기사에 무관한 부동산 기사 조각이 안 잘리고 섞여 들어가서,
    # HCX가 그 무관한 문장을 이 기사의 claim으로 잘못 추출 -> KOSIS 표까지 잘못 매칭돼
    # HIGH로 확정된 사례 발견). 해시태그 1개 + "더보기"가 근처에 오는 패턴도 추가로 잡는다.
    r"|#\S+(?:\s+\S+){0,6}\s*더보기"
    r"|구독수\s*\d+"
    # 2026-08-31 추가: 기사 끝에 붙는 "기자 소개/약력" 위젯이 안 잘리는 사례 추가 실측
    # 확인(row294/골든셋 5번·19번) — "OOO 기자 [부서명]. [출입처/경력]..." 패턴은
    # 실제 본문에서 "정석우 기자 정책팀장 경제부 정책팀장. 재정경제부·...를 출입하고
    # 있다"처럼 나타난다. 인용문 안의 "OOO 기자"(예: "김민수 한은 금융통계팀장은
    # ...라고 했다")와 구분하기 위해, 뒤에 실제 언론사 부서명이 바로 붙는 경우만 좁게 잡는다.
    r"|[가-힣]{2,4}\s*기자\s+(정책팀장|경제부|정치부|사회부|산업부|국제부|증권부|테크부)"
    # "오늘의 핫뉴스"/"English 기사보기" 위젯(골든셋 36번 실측) — 관련 없는 추천 기사
    # 헤드라인 목록으로 이어짐.
    r"|오늘의\s*핫뉴스|English\s*기사보기"
)
