...
# 2026-08-17: "20대"/"청년" 같은 나이 표현 처리 로직(_resolve_decade_age_codes 등)과
# 그걸 쓰던 resolve_decade_age_responses()는 여기서 제거했다 — 애초에 이 챗봇 파일 자신도
# 호출하는 곳이 없던 죽은 코드였고(grep 확인), 실제로 쓰는 곳은 batch_runner.py(뉴스
# 팩트체킹 파이프라인) 하나뿐이라 그쪽에 직접 옮겼다. 이 챗봇 기능 자체를 정리할 계획이라
# "공용 모듈"로 분리해 둘 이유도 없었다.
