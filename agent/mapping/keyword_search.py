...
def _expand_query_terms(sentence: str) -> set[str]:
    ...
    2026-08-18: _score_table의 카탈로그 키워드 매칭과 마찬가지로 _is_false_friend_only도
    같이 확인한다 — 안 그러면 "이사"(SYNONYMS) 같은 raw_term이 "이사회"/"대표이사"의
    일부로만 등장해도 그대로 "국내이동"/"인구이동"을 검색어에 섞어 넣게 된다."""
    terms: set[str] = set()
    for raw_term, mapped_terms in SYNONYMS.items():
        if raw_term in sentence and not _is_false_friend_only(raw_term, sentence):
...
# 2026-08-18: SYNONYMS의 "이사"(집을 옮김 뜻, "국내이동"/"인구이동" 동의어)도 같은 부류 —
# "이사회"/"대표이사"/"사외이사"/"이사장" 등 경제기사에 흔한 회사 지배구조 용어가 전부
# "이사"를 포함해서 오매칭 위험이 있다(kiwi로 확인해도 "대표이사"→"대표"+"이사"로 쪼개져
# "이사"가 독립 토큰으로 나오므로, "달"/"월" 때처럼 독립 토큰 여부만으로는 못 거른다 —
# "경유"와 같은 이유로 이 방식(구체적 복합어 등록)이 필요).
_FALSE_FRIEND_COMPOUNDS: dict[str, tuple[str, ...]] = {
    "유가": ("유가증권",),
    "경유": ("경유해", "경유하는", "경유했", "경유하고", "경유하여", "경유할", "경유지"),
    "증시": ("보증시",),
    "환율": ("순환율",),
    "이사": ("이사회", "대표이사", "사외이사", "이사장", "이사진", "상무이사", "전무이사", "이사직"),
}
